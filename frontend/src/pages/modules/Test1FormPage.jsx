/**
 * Test1FormPage.jsx  —  AUTO-GENERATED (2026-06-18T05:45:04.962Z)
...
const Test1FormPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (result.success) navigate('/test1');
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 animate-slide-up">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/test1')} className="btn-ghost">
      ...
      <div className="card p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          {/* Names */}
          <div className="">
            <label className="label">Names<span className="text-red-500 ml-0.5">*</span></label>
          <input type="text" {...register('names', { required: 'Names is required' })} placeholder="Enter Names..." className={clsx('input-field', errors.names && 'input-field-error')} />
            {errors.names && (
              <p className="error-message"><AlertCircle className="w-3.5 h-3.5" />{errors.names.message}</p>
            )}
          </div>
          </div>

          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            <button type="button" onClick={() => navigate('/test1')} className="btn-secondary" disabled={isSubmitting}>
              Cancel
            </button>

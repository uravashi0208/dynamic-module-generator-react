// Auto-generated controller for module: Basic Form
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [records, total] = await Promise.all([
      BasicForm.find().sort({ createdAt: -1 }).skip(skip).limit(parseInt(limit)).lean(),
      BasicForm.countDocuments(),
    ]);
...
exports.getOne = async (req, res, next) => {
  try {
    const record = await BasicForm.findById(req.params.id).lean();
...
exports.create = async (req, res, next) => {
  try {
    const record = await BasicForm.create({
      ...req.body,
...
exports.update = async (req, res, next) => {
  try {
    const record = await BasicForm.findByIdAndUpdate(
      req.params.id,
...
exports.remove = async (req, res, next) => {
  try {
    const record = await BasicForm.findByIdAndDelete(req.params.id);
    if (!record) return next(new AppError('Record not found.', 404));
